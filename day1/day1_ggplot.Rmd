...
- A flexible way of doing graphics in R
...
```{r}
library(ggplot2)
g <- ggplot(data = maltreat , aes(x=weight , y=weight))
```
...
```{r}
library(ggplot2)
g2 <- ggplot(data = maltreat , aes(x=sex , y=weight)) 
g2 + geom_boxplot(fill = "red")
```
...
```{r}
library(ggplot2)
g3 <- ggplot(data = maltreat , aes(x=sex ,y=weight ,fill=paying))  
g3 + geom_boxplot()
```
...
* * *
```{r, warning=FALSE,echo=T , size="tiny",echo=T}
library(ggplot2)
maltreat$paying<- factor(maltreat$paying,
levels = c("","Yes"),
labels = c("No", "Yes")) 
g4 <- ggplot(data = maltreat , aes(x=sex , y=resprate ,fill=paying)) 

g4 <- g4 +  geom_boxplot(position=position_dodge(.8)) 
g4 + ggtitle("A box blot of Resprate by gender") +  
    xlab("Gender") + ylab("Resprate") +  theme_bw() 
```
